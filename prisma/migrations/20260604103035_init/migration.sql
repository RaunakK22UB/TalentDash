-- CreateEnum
CREATE TYPE "LevelStandardized" AS ENUM ('L3', 'L4', 'L5', 'L6', 'SDE_I', 'SDE_II', 'SDE_III', 'STAFF', 'PRINCIPAL', 'IC4', 'IC5');

-- CreateEnum
CREATE TYPE "Currency" AS ENUM ('INR', 'USD', 'GBP', 'EUR');

-- CreateEnum
CREATE TYPE "TransportSource" AS ENUM ('CONTRIBUTOR', 'SCRAPED', 'AI_INFERRED');
...
CREATE TABLE "Salary" (
    ...
    "role" TEXT NOT NULL,
    "level" "LevelStandardized" NOT NULL,
...
ALTER TABLE "Salary" ADD CONSTRAINT confidence_score_range CHECK (confidence_score >= 0.0 AND confidence_score <= 1.0);
ALTER TABLE "Salary" ADD CONSTRAINT experience_years_range CHECK (experience_years > 0 AND experience_years < 51);
